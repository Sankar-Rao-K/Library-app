...
export default function IssueBook() {
  ...
  // Auto-focus barcode input when we reach that step
  useEffect(() => {
  ...
  // Step 2: Barcode scanned — find book
  const handleBarcodeSubmit = async (e) => {
    ...
    try {
      ...
      if (!found) {
        setError("No book found with this barcode. Please try again.");
        setBarcode("");
        barcodeRef.current?.focus();
      } else if (!found.available) {
        setError(`"${found.title}" is already issued to another student.`);
        setBarcode("");
        barcodeRef.current?.focus();
      } else {
        setBook(found);
    ...
    <AdminLayout>
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Enter student PIN, then scan the book barcode.
        </p>
      </div>
        ...
        {step === STEPS.PIN && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-400 mb-5">
              Ask the student for their PIN number.
            </p>
            <form onSubmit={handlePinSubmit} className="space-y-4">
              <input
                ...
                className="w-full border border-gray-300 rounded-lg px-4 py-3 text-2xl font-mono tracking-widest text-center focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
              <button
                type="submit"
                disabled={loading}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg px-4 py-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-green-600">PIN: {student.pin} · Class: {student.class}</p>
              </div>
            </div>

            <h2 className="text-lg font-semibold text-gray-800 mb-1">Scan Book Barcode</h2>
            <p className="text-sm text-gray-400 mb-5">
              Click the field below and scan the barcode with your scanner.
            </p>
            <form onSubmit={handleBarcodeSubmit} className="space-y-4">
              <div className="relative">
                <input
                  ...
                  className="w-full border-2 border-blue-400 rounded-lg px-4 py-3 text-lg font-mono text-center focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
                <span className="absolute right-4 top-3.5 text-gray-300 text-xl">
                  📷
                </span>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {book.author} · Barcode: {book.barcode}
                  </p>
                </div>
              </div>
            </div>
        ...
        {step === STEPS.SUCCESS && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
            ...
            <p className="text-gray-500 text-sm mb-2">
              ...
              issued to <span className="font-semibold text-gray-700">{student.name}</span>.
            </p>
            <p className="text-gray-400 text-xs mb-8">
              Transaction saved · Book marked as unavailable
            </p>
            <button
              onClick={reset}
