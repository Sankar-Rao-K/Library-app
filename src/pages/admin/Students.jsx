...
function parseStudentsFromWorkbook(workbook) {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    let headerIdx = -1, cols = { pin: 1, name: 2 };
    ...
    for (let i = dataStart; i < rows.length; i++) {
      ...
      const pin = row[cols.pin], name = row[cols.name];
      if (!pin || !name) continue;
      if (String(pin).toLowerCase().includes("pin")) continue;
      const pinStr = String(pin).trim(), nameStr = String(name).trim();
  ...
  return (
    <tr className="bg-blue-50 border-l-4 border-blue-500">
      <td colSpan={5} className="px-4 py-4">
        <p className="text-xs font-bold text-blue-700 uppercase tracking-wide mb-3">✏️ Editing: {student.name}</p>
        <form onSubmit={handleSave} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1">Full Name</label>
            <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Full name" className={inp} autoFocus />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1">PIN Number</label>
            <input required value={form.pin} onChange={(e) => setForm({ ...form, pin: e.target.value })} placeholder="e.g. 23173-CM-001" className={`${inp} font-mono`} />
          ...
          <div className="sm:col-span-3 flex gap-2 pt-1">
            <button type="submit" disabled={saving} className="text-xs font-bold px-4 py-2 rounded-lg text-white disabled:opacity-50" style={{ background: "linear-gradient(135deg, #0D1F4E, #1B4332)" }}>
              {saving ? "Saving…" : "✓ Save Changes"}
            </button>
            <button type="button" onClick={onCancel} className="text-xs font-medium px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100">Cancel</button>
          </div>
        </form>
      </td>
    </tr>
...
const SEM_ORDER = ["I Year — Sem 1","II Year — Sem 3","II Year — Sem 4","III Year — Sem 5","III Year — Sem 6","Passed Out","Unknown"];
...
export default function Students() {
  ...
  const handleSaveEdit = async (studentId, formData, branch, yearLabel, sem, isOld) => {
    ...
    await updateStudent(studentId, { name: formData.name.trim(), pin: formData.pin.trim(), email: formData.email.trim(), branch, year: yearLabel, currentSem: sem, semNum, isOld });
    setEditingId(null);
  };
  ...
  // Dropdown results — top 6 for instant navigation
  const dropdownResults = rawSearch.trim().length >= 2
    ? smartSearch(students, rawSearch.trim(), ["name", "pin", "branch"]).slice(0, 6)
    : null;

  const renderDropdownResult = (s) => {
    const { yearLabel, isOld } = getStudentInfo(s.pin);
    return (
      <div className="flex items-center gap-3 px-4 py-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 font-mono">
            <HL text={s.pin} query={rawSearch} /> · {s.branch}
          </p>
        </div>
        <span className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${isOld ? "bg-gray-100 text-gray-500" : "bg-blue-50 text-blue-700"}`}>
  ...
  return (
    <AdminLayout>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-6">
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Ravi Kumar"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">PIN Number</label>
              <input type="text" required value={form.pin} onChange={(e) => setForm({ ...form, pin: e.target.value })} placeholder="e.g. 23173-CM-001"
            ...
            <div>
              ...
              <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="e.g. ravi@school.com"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
      ...
      {showImport && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-6">
          <h2 className="text-base font-semibold text-gray-800 mb-1">Import Students</h2>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 flex items-start gap-3">
            <span className="text-xl flex-shrink-0">📄</span>
            <div className="flex-1">
              <p className="text-sm font-bold text-amber-800">New to importing?</p>
              <p className="text-xs text-amber-600 mt-0.5 mb-3">Download the template, fill in PIN and Name, then upload.</p>
              ...
              <div className="overflow-x-auto max-h-64 overflow-y-auto border border-gray-100 rounded-lg">
                <table className="w-full text-xs">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr className="text-left text-gray-500"><th className="px-3 py-2">#</th><th className="px-3 py-2">PIN</th><th className="px-3 py-2">Name</th><th className="px-3 py-2">Branch</th><th className="px-3 py-2">Year</th><th className="px-3 py-2">Act</th></tr>
                  </thead>
                      ...
                      <tr key={idx} className={editIdx === idx ? "bg-blue-50" : "hover:bg-gray-50"}>
                        ...
                        {editIdx === idx ? (
                          <>
                            ...
                            <td className="px-3 py-2 text-gray-600">{row.branch}</td><td className="px-3 py-2 text-gray-600">{row.year}</td>
                            <td className="px-3 py-2"><button onClick={() => setEditIdx(null)} className="text-green-600 font-medium">Done</button></td>
                          </>
                        ) : (
                          <>
                            <td className="px-3 py-2 font-mono text-gray-600">{row.pin}</td><td className="px-3 py-2 font-medium text-gray-800">{row.name}</td>
                            <td className="px-3 py-2 text-gray-500">{row.branch}</td><td className="px-3 py-2 text-gray-500">{row.year}</td>
      ...
      <SearchBar
        ...
        onResultClick={(s) => { setRawSearch(""); setSearch(""); setSelectedStudent(s); }}
        emptyMessage={isId ? `No student with PIN "${rawSearch}"` : "No students found"}
        className="mb-2"
      />
      ...
      {rawSearch.trim().length >= 2 && (
        <p className="text-xs mb-4 pl-1">
          {isId
            ? <span className="text-amber-600 font-medium">🔑 PIN search — exact matches only · {filtered.length} result{filtered.length !== 1 ? "s" : ""}</span>
            : <span className="text-gray-400">🔍 Smart search · {filtered.length} of {students.length} students</span>}
        </p>
      )}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 py-16 text-center">
          ...
          <p className="font-medium text-gray-600 text-sm">
            {isId ? `No student with PIN "${rawSearch}"` : searchActive ? `No match for "${rawSearch}"` : "No students added yet"}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            {isId ? "PIN must match exactly. Format: 23173-CM-001" : searchActive ? "Try different keywords." : ""}
          </p>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {group.map((s) => (
                        <>
                          {editingId === s.id ? (
                            <StudentEditForm key={`edit-${s.id}`} student={s}
                              onSave={(fd, branch, yl, sem, io) => handleSaveEdit(s.id, fd, branch, yl, sem, io)}
                              onCancel={() => setEditingId(null)} />
                          ) : (
                            <tr key={s.id} className="hover:bg-gray-50 transition">
                              <td className="px-5 py-3 font-medium text-gray-800 cursor-pointer hover:text-blue-600" onClick={() => setSelectedStudent(s)}>
                                <HL text={s.name} query={search} />
                              </td>
                              <td className="px-5 py-3 font-mono text-xs text-gray-500">
                                <HL text={s.pin} query={search} />
                              </td>
                  ...
                  {group.map((s) => (
                    <div key={s.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                      <div className="flex items-center justify-between gap-3" onClick={() => setSelectedStudent(s)}>
                        <div className="flex items-center gap-3 min-w-0 cursor-pointer">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-400 font-mono truncate"><HL text={s.pin} query={search} /></p>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-100 flex gap-2">
                        <button onClick={() => setSelectedStudent(s)} className="flex-1 text-xs font-medium py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50">View</button>
                        <button onClick={() => setEditingId(s.id)} className="flex-1 text-xs font-medium py-1.5 rounded-lg border border-green-200 text-green-700 hover:bg-green-50">✏️ Edit</button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {selectedStudent && (
        <StudentDetailModal student={selectedStudent} onClose={() => setSelectedStudent(null)} onDeleted={() => setSelectedStudent(null)} />
      )}
    </AdminLayout>
  );
}