...
import { smartSearch, isIdQuery, debounce } from "../../utils/searchUtils";
...
function getDateRange(period, customStart, customEnd, specificDate) {
  if (specificDate) return { start: new Date(specificDate + "T00:00:00"), end: new Date(specificDate + "T23:59:59") };
  const now = new Date();
...
function matchesDaysFilter(t, key) {
...
function downloadCSV(data, filename) {
  ...
  const rows = data.map((t, i) => {
    ...
    const days = id ? Math.floor(((rd || new Date()) - id) / 86400000) : "";
    return [i+1, t.bookTitle||"", t.barcode||"", t.studentName||t.borrowerName||"",
      t.studentPin||t.borrowerId||"", t.borrowerType||"student",
      id?id.toLocaleDateString("en-IN"):"", rd?rd.toLocaleDateString("en-IN"):"", t.status||"", days];
  });
...
export default function Reports() {
  ...
  const debouncedSet = useCallback(debounce((v) => setSearchTerm(v), 200), []);
  ...
  const isId = isIdQuery(searchTerm.trim());

  // Active filter count
  const activeFilters = [
    statusFilter !== "all", typeFilter !== "all",
    daysFilter !== "all", searchTerm.trim().length > 0, !!specificDate,
  ].filter(Boolean).length;

  const clearAllFilters = () => {
    setStatusFilter("all"); setTypeFilter("all"); setDaysFilter("all");
    setRawSearch(""); setSearchTerm(""); setSpecificDate("");
  };
  ...
  const handlePrint = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Library Report</title><style>
      body{font-family:Arial;margin:0;padding:20px;font-size:12px}
  ...
  const pill = (active, danger = false) =>
    `px-3.5 py-1.5 rounded-full text-xs font-semibold transition border ${
      active
        ? danger ? "bg-red-600 text-white border-red-600" : "text-white border-transparent"
    ...
    <AdminLayout>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => downloadCSV(filtered, `Library_Report_${period}_${todayStr()}.csv`)}
          ...
          <button onClick={handlePrint}
            className="flex items-center gap-1.5 text-white px-4 py-2 rounded-lg text-sm font-semibold transition" style={ACTIVE}>
            🖨️ Print
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div key={label} className={`${bg} border ${border} rounded-xl p-4 flex items-center gap-3`}>
            <div><p className={`text-2xl font-bold ${color}`}>{value}</p><p className="text-xs text-gray-500 mt-0.5">{label}</p></div>
          </div>
        ))}
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {PERIODS.map((p) => (
            <button key={p.key}
              ...
              style={period === p.key && !specificDate ? ACTIVE : {}}>
              {p.label}
            </button>
          ))}
          <div className="flex items-center gap-2 ml-auto">
            ...
            {specificDate && <button onClick={() => setSpecificDate("")} className="text-xs text-red-500 hover:underline">Clear</button>}
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <button onClick={() => setShowFilters(!showFilters)}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition border ${
            showFilters || activeFilters > 0 ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-200 hover:border-blue-400"
          }`}>
          <span>{showFilters ? "▲" : "▼"}</span>
          <span>Filters</span>
          {activeFilters > 0 && <span className="bg-white/25 text-white text-xs px-1.5 py-0.5 rounded-full font-bold">{activeFilters}</span>}
        </button>
        ...
        {daysFilter !== "all" && (
          <span className="flex items-center gap-1 text-xs bg-amber-50 text-amber-700 border border-amber-200 px-2.5 py-1 rounded-full font-medium">
            ⏱ {daysFilter} days
            <button onClick={() => setDaysFilter("all")} className="ml-1">✕</button>
          </span>
        )}
        {searchTerm.trim() && (
          <span className="flex items-center gap-1 text-xs bg-gray-100 text-gray-700 border border-gray-200 px-2.5 py-1 rounded-full font-medium">
            🔍 "{rawSearch}"
            <button onClick={() => { setRawSearch(""); setSearchTerm(""); }} className="ml-1">✕</button>
          </span>
        )}
        {activeFilters > 0 && (
          <button onClick={clearAllFilters} className="text-xs text-red-500 hover:underline font-medium ml-auto">Clear all</button>
        )}
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-6">
            <div>
              ...
              <div className="flex gap-2">
                {[{ key:"all", label:"All" }, { key:"issued", label:"📤 Issued" }, { key:"returned", label:"📥 Returned" }].map(({ key, label }) => (
                  <button key={key} onClick={() => setStatusFilter(key)}
                    className={pill(statusFilter === key)} style={statusFilter === key ? ACTIVE : {}}>{label}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                {[{ key:"all", label:"All" }, { key:"student", label:"🎓 Students" }, { key:"staff", label:"👩‍🏫 Staff" }].map(({ key, label }) => (
                  <button key={key} onClick={() => setTypeFilter(key)}
                    className={pill(typeFilter === key)} style={typeFilter === key ? ACTIVE : {}}>{label}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-gray-50">
            ...
            <div className="flex flex-wrap gap-2">
              {DAYS_FILTERS.map(({ key, label, danger }) => (
                <button key={key} onClick={() => setDaysFilter(key)}
                  className={pill(daysFilter === key, danger)}
                  style={!danger && daysFilter === key ? ACTIVE : {}}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div className="pt-3 border-t border-gray-50">
            ...
            <SearchBar
              ...
              placeholder="Book title, borrower name, accession, or ID…"
              resultCount={searchTerm.trim() ? filtered.length : null}
              totalCount={null}
              minChars={2}
              isIdSearch={isId}
              className="max-w-lg"
            ...
            {searchTerm.trim() && (
              <p className="text-xs mt-1.5 pl-1">
                {isId
                  ? <span className="text-amber-600 font-medium">🔑 ID search — exact matches</span>
                  : <span className="text-gray-400">🔍 Smart search · {filtered.length} result{filtered.length !== 1 ? "s" : ""}</span>}
              </p>
            )}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            {activeFilters > 0 && (
              <button onClick={clearAllFilters} className="mt-3 text-xs text-blue-600 hover:underline font-medium">Clear all filters</button>
            )}
          </div>
        ) : (
                ...
                {filtered.map((t, i) => {
                  ...
                  return (
                    <tr key={t.id} className={`transition ${isOverdue ? "bg-red-50/40" : "hover:bg-gray-50"}`}>
                      ...
                      <td className="px-4 py-3 font-medium text-gray-800 max-w-xs"><p className="truncate">{t.bookTitle}</p></td>
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${(t.borrowerType||"student")==="staff" ? "bg-indigo-100 text-indigo-700" : "bg-blue-50 text-blue-700"}`}>
                      ...
                      <td className="px-4 py-3 text-xs">
                        {days !== null ? <span className={isOverdue ? "text-red-600 font-bold" : "text-gray-500"}>{days}d{isOverdue ? " ⚠️" : ""}</span> : "—"}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${t.status === "issued" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}>
                          {t.status}
                        </span>
                      </td>
                    </tr>
