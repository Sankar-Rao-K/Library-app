...
import {
  ...
  getTransactionsByBorrower, checkAndDeletePassedOutStudent,
...
export default function ReturnBook() {
  ...
  // Set when a passed-out student is auto-deleted on clear
  const [autoDeleted, setAutoDeleted]= useState(false);
  const barcodeRef = useRef(null);

  // Auto-fetch when navigated with prefillPin
  useEffect(() => {
  ...
  const resetAll = () => {
    ...
    setIssuedBooks([]); setError(""); setAutoDeleted(false);
  };

  // ── Book-only reset — keeps student, refreshes issued list ───────────
  const resetBookOnly = async () => {
    setBarcode(""); setBook(null); setTransaction(null);
    setError(""); setAutoDeleted(false);
    if (student) await loadIssuedBooks(student.id);
    setStep(STEPS.BARCODE);
  };

  // ── Back from BARCODE → PIN (keeps pin value) ─────────────────────────
  const goBackToPin = () => {
    setBarcode(""); setBook(null); setTransaction(null);
    setIssuedBooks([]); setError(""); setAutoDeleted(false);
    setStep(STEPS.PIN);
  ...
  // Tap a listed book → auto-fill barcode input
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // ── Auto-delete if student is passed out and now has zero dues ───
      const wasDeleted = await checkAndDeletePassedOutStudent(
        student.id, student.pin
      );
      setAutoDeleted(wasDeleted);
  ...
  // ── Semester badge for the student ────────────────────────────────────
  const { label: semLabel, isPassedOut } = student
    ? getStudentInfo(student.pin)
    : { label: "", isPassedOut: false };
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              {isPassedOut && (
                <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-700">
                  ⚠️ This student has passed out. After all dues are cleared, their record will be automatically removed.
                </div>
              )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <form onSubmit={handleBarcodeSubmit} className="space-y-3">
                ...
                <div className="flex gap-3">
                  ...
                  <button type="submit"
                    disabled={loading || issuedBooks.length === 0}
        ...
        {step === STEPS.CONFIRM && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-5 mb-6 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    ...
                    {semLabel && <span className="ml-2 text-xs text-blue-600">({semLabel})</span>}
                  </p>
                </div>
              </div>
              ...
              {/* Passed-out warning on confirm */}
              {isPassedOut && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-700">
                  ⚠️ Passed-out student — if this is their last book, their record will be auto-removed after confirmation.
                </div>
              )}
            </div>
            <div className="flex gap-3">
              <button onClick={() => {
                setStep(STEPS.BARCODE); setBarcode("");
                setBook(null); setTransaction(null); setError("");
              }}
                className="flex-1 border border-gray-300 text-gray-600 py-3 rounded-lg font-medium hover:bg-gray-50 transition">
                ← Back
              </button>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
            ...
            {autoDeleted && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-5 text-sm text-blue-700">
                <p className="font-bold mb-0.5">✅ Student record auto-removed</p>
                <p className="text-xs text-blue-500">
                  {student?.name} has passed out and cleared all dues —
                  their record has been automatically deleted.
                </p>
              </div>
            )}

            <div className="flex flex-col sm:flex-row gap-3">
              {/* Return another book for same student — only shown if NOT auto-deleted */}
              {!autoDeleted && (
                <button onClick={resetBookOnly}
              ...
              <button onClick={resetAll}
                className={`text-gray-600 hover:bg-gray-50 py-3 rounded-xl font-bold text-sm transition border-2 border-gray-300 ${autoDeleted ? "w-full" : "flex-1"}`}>
                {autoDeleted ? "✓ Done" : "👤 New Student"}
                {!autoDeleted && (
