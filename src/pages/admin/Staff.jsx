...
export default function Staff() {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      setImportSummary(
        dupes.length > 0
          ? `✅ ${newOnes.length} imported. ⚠️ ${dupes.length} skipped (ID exists): ${dupes.map(s => s.staffId).slice(0, 5).join(", ")}${dupes.length > 5 ? "..." : ""}`
  ...
  return (
    <AdminLayout>
      ...
      {showImport && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-6">
          ...
          <p className="text-xs text-gray-400 mb-3">
            Section auto-detected from row headers. Supports .xlsx · .csv · .json
          </p>
      ...
      {/* Search */}
      <div className="mb-5">
        <input type="text" placeholder="Search by name, staff ID, designation, or section..."
