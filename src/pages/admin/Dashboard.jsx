...
// ═══════════════════════════════════════════════════════════════════════
// UNIVERSAL SEARCH  — tabbed, debounced, highlighted, field-attributed
// ═══════════════════════════════════════════════════════════════════════
...
function UniversalSearch({ students, staff, books, transactions, onNavigate }) {
  const [rawQuery, setRawQuery]   = useState("");   // what the user typed
  const [query, setQuery]         = useState("");   // debounced value used for search
  ...
  useEffect(() => {
    const handler = (e) => { if (e.key === "Escape") handleClear(); };
  ...
  const bookResults = q.length >= 2
    ? smartSearch(books, q, ["title", "author", "accessionNo", "barcode", "subject"], 20, [], 50)
    : [];
  const studentResults = q.length >= 2
    ? smartSearch(students, q, ["name", "pin", "branch", "year"], 20, [], 50)
    : [];
  const staffResults = q.length >= 2
    ? smartSearch(staff, q, ["name", "staffId", "section", "designation"], 20, [], 50)
    : [];
  const txnResults = q.length >= 2
    ? smartSearch(transactions, q, ["bookTitle", "studentName", "borrowerName", "studentPin", "borrowerId"], 20, [], 30)
    : [];
  ...
  const duesFor = (id) =>
    transactions.filter((t) => (t.borrowerId === id || t.studentId === id) && t.status === "issued").length;
  ...
  // ── Book result row ───────────────────────────────────────────────
  const BookRow = ({ b }) => {
    const matchedField = getMatchedField(b, q, "title", [
      { field: "author",      label: "author"    },
      { field: "accessionNo", label: "accession" },
      { field: "barcode",     label: "accession" },
    ...
    return (
      <button
        ...
        <div className="flex-1 min-w-0">
          ...
          {matchedField && (
            <span className="text-xs text-purple-600 font-medium bg-purple-50 px-1.5 py-0.5 rounded mt-0.5 inline-block">
              matched: {matchedField.label} {matchedField.value}
            </span>
          )}
        </div>
      ...
      <button
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 font-mono">
            <Highlight text={s.pin} query={q} /> · {s.branch} · {s.year}
          </p>
        </div>
        <span className={`flex-shrink-0 px-2.5 py-0.5 rounded-full text-xs font-bold ${
      ...
      <button
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">
            {s.designation} · {s.section} · <span className="font-mono"><Highlight text={s.staffId} query={q} /></span>
          </p>
        </div>
        <span className={`flex-shrink-0 px-2.5 py-0.5 rounded-full text-xs font-bold ${
          dues > 0 ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"
  ...
  const TxnRow = ({ t }) => {
    ...
    return (
      <button
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-800 truncate">
            <Highlight text={t.bookTitle} query={q} />
          </p>
          <p className="text-xs text-gray-400 truncate">
            <Highlight text={t.studentName || t.borrowerName || ""} query={q} />
  ...
  // ── "All" tab: top 3 per category with "+N more" ──────────────────
  const AllResults = () => {
    const sections = [
      { key: "books",        label: "📚 Books",        results: bookResults,    Row: BookRow    },
      { key: "students",     label: "🎓 Students",     results: studentResults, Row: StudentRow },
      { key: "staff",        label: "👩‍🏫 Staff",       results: staffResults,   Row: StaffRow   },
      { key: "transactions", label: "📋 Transactions", results: txnResults,     Row: TxnRow     },
        ...
        {sections.map(({ key, label, results, Row }) => (
          <div key={key}>
            <div className="px-4 py-1.5 flex items-center justify-between"
              style={{ background: "rgba(219,234,254,0.55)", backdropFilter: "blur(4px)" }}>
              <span className="text-xs font-bold text-blue-700 uppercase tracking-wider">{label}</span>
              <span className="text-xs text-blue-500 font-medium">{results.length} result{results.length !== 1 ? "s" : ""}</span>
            </div>
            {results.slice(0, 3).map((item) => (
              <Row key={item.id} {...(key === "books" ? { b: item } : key === "students" ? { s: item } : key === "staff" ? { s: item } : { t: item })} />
            ))}
            {results.length > 3 && (
              <button
                onMouseDown={(e) => { e.preventDefault(); setActiveTab(key); }}
  ...
  const CategoryResults = () => {
    const map = {
      books:        { results: bookResults,    Row: BookRow,    empty: "No books" },
      students:     { results: studentResults, Row: StudentRow, empty: "No students" },
      staff:        { results: staffResults,   Row: StaffRow,   empty: "No staff" },
      transactions: { results: txnResults,     Row: TxnRow,     empty: "No transactions" },
    };
      ...
      return (
        <div className="px-4 py-8 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">
            {idMode ? "ID must match exactly" : "Try different keywords"}
          </p>
        </div>
      );
    }
    ...
    return (
      <div>
        {results.map((item) => (
          <Row key={item.id} {...(activeTab === "books" ? { b: item } : activeTab === "students" ? { s: item } : activeTab === "staff" ? { s: item } : { t: item })} />
        ))}
        {results.length >= 50 && (
    ...
    <div ref={containerRef} className="relative mb-6">
      ...
      <div className={`bg-white transition-all duration-200 ${
        ...
        <div className="flex items-center gap-3 px-5 py-3.5">
          <svg className={`w-5 h-5 flex-shrink-0 transition-colors ${open ? "text-blue-500" : "text-gray-400"}`}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {open && totalCount > 0 && (
              <span className="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full tabular-nums">
                {totalCount > 150 ? "150+" : totalCount}
              </span>
            )}
            {idMode && rawQuery && (
      ...
      {rawQuery.trim().length === 1 && (
        <div className="absolute left-0 right-0 z-50 border border-t-0 border-blue-300 rounded-b-2xl bg-blue-50 px-4 py-3 text-center"
          style={{ boxShadow: "0 8px 24px rgba(59,130,246,0.12)" }}>
      ...
      {open && rawQuery.trim().length >= 2 && (
        <div
          ...
          {/* Tab bar */}
          <div className="flex items-center gap-1 px-3 py-2 border-b border-blue-100 overflow-x-auto"
            ...
            <div className="ml-auto flex-shrink-0">
              <button
                ...
                className="text-xs text-blue-400 hover:text-blue-600 px-2 py-1 transition">
                Esc ✕
              </button>
            </div>
          </div>
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {!hasResults ? (
              <div className="px-4 py-8 text-center">
                ...
                <p className="text-sm font-semibold text-gray-600">
                  {idMode ? `No exact match for "${q}"` : `No results for "${q}"`}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  {idMode
          ...
          <div className="px-4 py-2 text-center border-t border-blue-100"
            ...
            <p className="text-xs text-blue-400">
              Click a result to navigate · Esc to close
            </p>
          </div>
        </div>
      )}
...
// ═══════════════════════════════════════════════════════════════
// TRANSACTION TABLE — one clean consolidated filter row
// ═══════════════════════════════════════════════════════════════
function TransactionTable({ transactions, onViewIssued }) {
  ...
  const pill   = (active) =>
    `px-3 py-1.5 rounded-md text-xs font-semibold transition ${active ? "" : "text-gray-500 hover:text-gray-700"}`;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="flex flex-col gap-3 px-5 py-4 border-b border-gray-100"
        ...
        {/* ONE consolidated filter row — type + status together */}
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center bg-gray-100 rounded-lg p-1 gap-0.5">
            {[
              ...
              <button key={key} onClick={() => { setTypeFilter(key); setLimit(20); }}
                className={pill(typeFilter === key)}
                style={typeFilter === key ? ACTIVE : {}}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center bg-gray-100 rounded-lg p-1 gap-0.5">
            {[
              ...
              <button key={key} onClick={() => { setStatusFilter(key); setLimit(20); }}
                className={pill(statusFilter === key)}
                style={statusFilter === key ? ACTIVE : {}}>
                {label}
              </button>
            ))}
          </div>
          ...
          {(typeFilter !== "all" || statusFilter !== "all") && (
            <button
              onClick={() => { setTypeFilter("all"); setStatusFilter("all"); setLimit(20); }}
                ...
                <div key={t.id} className="px-5 py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {t.studentName || t.borrowerName}{days !== null ? ` · ${days}d` : ""}
                    </p>
                  </div>
